...
def phenom_correlat(df_PCA_nlz, ukbb_y, y_desc_dict, y_cat_dict, reorder = True):
    ...
    comp_nlz = df_PCA_nlz.columns.values
...
def findFDR_alt(clreddf, col, thresBon):
    """
    Finds FDR threshold in the standard method
    As used in the sklearn function, but modified to determine
    the p value threshold in real terms
...
def printing(clreddf, lk, y_desc_dict, y_cat_dict,
    ...
    for catn in np.unique(sigcol['true_catid']):
        if catn == 31:
            catn = 21
        sigcol_subset = sigcol[sigcol['true_catid'] == catn]
        if sort:
...
def translate(cols, y_cat_dict):
    """
    Translates columns from phesid
    To something that is understandable by
    y_cat_dict and y_descr_dict
    """
    if not isinstance(cols, (list, np.ndarray)):
        cols = [cols]
    new_cs = []
...
def hits_csv(clreddf, lk, y_desc_dict, y_cat_dict, OUT_FOLD, 
             thres=0.05, thresBon=None, useFDR=True, sort=False):

    hits_keys = ['i', 'Column', 'Option', 'phesid', '-log10(p)', 'r', 'description']
    ...
    with pd.ExcelWriter(os.path.join(OUT_FOLD, f'hits_{lk}_N{len(sigcol)}_{sub}.xlsx')) as writer:  
        ...
        for catn in np.unique(sigcol['catid']):
            ...
            cat_name = y_cat_dict[y_cat_dict['Cat_ID'] == catn]['Cat_Name'][0]

            # Excel has quirks and name limits
            # so trim down the cat name
            cat_name = cat_name.replace(' and environment', '').replace('measures ', '')
            cat_name = cat_name.replace(' and ', '+').replace(' & blood vessels', '')
            cat_name = cat_name.replace('+sizes', '').replace('/', ' or ')
            ...
            if len(cs) > 0:
                ...
                for ic, c in enumerate(cs):
                    ...
                    col_data['Column'].append(sigcol_subset['coid'].iloc[ic])
                    col_data['Option'].append(sigcol_subset['type'].iloc[ic])
                    col_data['phesid'].append(sigcol_subset['phesid'].iloc[ic])
                    col_data['-log10(p)'].append(sigcol_subset[f'-logp_{lk}'].iloc[ic])
                    col_data['r'].append(sigcol_subset[f'r_{lk}'].iloc[ic])
                    desc = y_desc_dict[new_cs[ic]]
                    col_data['description'].append(desc)
